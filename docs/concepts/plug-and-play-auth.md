...
In earlier beta releases, apps were required to integrate with the {{config.extra.arcana.sdk_name}} onboard users with custom login UI. For instance, to onboard users via Google, the custom login UI code was required to call the `loginWithSocial('google')` method. Similarly, the passwordless login option was required to provision an email input field and call `loginWithLink` in the SDK.
