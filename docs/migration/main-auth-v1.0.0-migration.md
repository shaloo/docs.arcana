...
If you wish to deploy your app on {{config.extra.arcana.company_name}} Testnet, follow steps 1 and 3.  To deploy your app on {{config.extra.arcana.company_name}}  Mainnet, follow all three steps:
...
3. **Install & Integrate**: Install and upgrade the {{config.extra.arcana.sdk_name}} to v1.0.0. Integrate the {{config.extra.arcana.sdk_name}} and create a new `AuthProvider` instance by specifying the **{{config.extra.arcana.app_address}}**. If you wish to deploy your app on the {{config.extra.arcana.company_name}} Testnet, provide the **{{config.extra.arcana.app_address}}** listed in the 'Testnet' configuration profile of your app in the dashboard. Also, you need to set the `network` parameter in the `AuthProvider` constructor to `testnet`. This is important because, by default, {{config.extra.arcana.sdk_name}} v1.0.0 has the `network` parameter set to using 'mainnet'. Similarly, if you want to deploy your app on the {{config.extra.arcana.company_name}}  Mainnet, then use the **{{config.extra.arcana.app_address}}** assigned to the 'Mainnet' configuration profile of your app in the dashboard and set the `network` parameter to 'mainnet'. 
...
For more details, see [[rn-main-auth-v1.0.0|{{config.extra.arcana.sdk_name}} v1.0.0 release notes]] for details.
