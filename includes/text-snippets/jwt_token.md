??? an-tip "Arcana JWT Token"
    ...
    === "`userDIDToken`"
        ...
        Upon successful authentication, {{config.extra.arcana.sdk_name}} returns a unique DID token to the app called the [[concept-arcana-jwt-token|Arcana DID Token]]. App developers can access this token via `getUser()` method and refer to the `userDIDToken` field of the [`UserInfo`](https://arcana-auth.netlify.app/interfaces/userinfo) object. 
